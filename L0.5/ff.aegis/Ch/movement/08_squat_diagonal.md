...
**L0.5 · Movement · Aegis** — 在锁定低步态蹲姿下按角度、速度和时长进行对角线移动，随后保持蹲姿。
...
| 参数 | 类型 / 范围 | 默认值 | 状态 | 含义 |
| --- | --- | --- | --- | --- |
| `angle_deg` | `float [-180, 180], non-cardinal` | — | 可用 | 以正前方为基准的角度，单位为度。 |
| `speed_mps` | `float > 0, component-limited` | — | 可用 | 速度，单位为米每秒；方向和有效范围以本行类型定义为准。 |
...
在锁定低步态蹲姿下按角度、速度和时长进行对角线移动，随后保持蹲姿。
<!-- END: Behavior -->
