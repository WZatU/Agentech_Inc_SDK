...
**L0.5 · Movement · Aegis** — 在保持锁定低步态蹲姿的情况下向左或向右行走，停止后继续保持蹲姿。
...
| 参数 | 类型 / 范围 | 默认值 | 状态 | 含义 |
| --- | --- | --- | --- | --- |
| `direction` | `enum {left, right}` | — | 可用 | 动作方向。 |
| `speed_mps` | `float [0.10, 1.00] m/s` | — | 可用 | 速度，单位为米每秒；方向和有效范围以本行类型定义为准。 |
...
在保持锁定低步态蹲姿的情况下向左或向右行走，停止后继续保持蹲姿。
<!-- END: Behavior -->
