# `Agentech.squat_forward()`

<!-- START: Definition -->
## Definition

**L0.5 · Movement · Aegis** — Walk forward in the latched low-gait squat stance, then stop and remain squatted.
...
```python
Agentech.squat_forward(speed_mps=0.5, duration_s=2.0)
```
<!-- END: Syntax -->
...
| --- | --- | --- | --- | --- |
| `speed_mps` | `float [0.05, 3.00] m/s` | — | Available | Required positive forward speed magnitude. Negative values and values outside the inclusive range are rejected. |
| `duration_s` | `float (0, 10] seconds` | — | Available | Required time for the low-gait forward command. |
<!-- END: Parameters -->

<!-- START: Behavior -->
## Behavior

Walk forward in the latched low-gait squat stance, then stop and remain squatted.
<!-- END: Behavior -->
