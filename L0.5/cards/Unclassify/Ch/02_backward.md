...
**L0.5 · 移动** — 在机身坐标系发送有时限的负向 `vx` 指令，结束时发送零运动指令。
...
1. 公开速度参数都是正的速度大小，封装层负责加上负 `vx` 符号。
2. 速度表达只能选择 `speed_mps`、`speed_percent`、`speed_level` 或 `pace` 中的一种；`distance_m` 只能和 `speed_mps` 组合。
...
| `Agentech.backward(distance_m=...)` | `speed_mps=0.4`；时长为 `distance_m / 0.4` |
<!-- END: Defaults -->

<!-- START: Parameters -->
## 参数

| 参数模式 | 范围 / 可选值 | 解析后的速度大小 |
| --- | --- | --- |
...
| `distance_m` | `0 < distance_m <= 3.0` | 时长 = `distance_m / speed_mps` |
| `duration_s` | `0 < duration_s <= 10.0` | 指令持续时间 |

ZSL-1 底层接受的非零 `|vx|` 为 `0.05..3.0 m/s`；Agentech 公开封装范围为 `0.05..1.0 m/s`。这些别名是确定性的 Agentech 映射，不是上游参数，也不是实测速度。
...
解析后的指令为 `move(-speed_mps, 0, 0)`；到达时长后发送 `move(0, 0, 0)`。模拟器必须接收完全相同的数值和时序。
<!-- END: Behavior -->
